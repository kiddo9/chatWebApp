...
class Messages extends Component
{
    ...
    public function openChatSpace($friendId){
        ...
        $chatRoom = chatRoom::where(function ($query) use ($auth, $friendId){
            $query->where('user_id1', $auth)->where('user_id2', $friendId);
        })->orWhere(function ($query) use ($auth, $friendId){
            $query->where('user_id1', $friendId)->where('user_id2',$auth );
        })->first();
        ...
        $this->chatSpaceid = $chatRoom->roomId;
        $this->friendId = $friendId;

        $this->dispatch('cred', FriendId:$this->friendId, chatSpaceId:$this->chatSpaceid);
